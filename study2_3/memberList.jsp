	...
	<div class="container">
		...
		<div class="row" style="margin-bottom: 10px;">
			<div class="col-sm-3 form-inline">
				${searchVO.totalRowCount }건 조회 <select id="id_rowSizePerPage"
					name="rowSizePerPage" class="form-control input-sm">
					<c:forEach var="i" begin="10" end="50" step="10">
						<option value="${i }"
							${seacrhVO.rowSizePerPage eq i ? "selected='selected'" : ""}>${i }</option>
					</c:forEach>
				</select>
			</div>
		</div>
...
<script type="text/javascript">
	...
	$('#id_btn_reset').click(
			function() {
				$form.find("select[name='searchHobby'] option:eq(0) ").attr(
						"selected", "selected");
				$form.find("select[name='searchJob'] option:eq(0) ").prop(
						"selected", "selected");
				$form.find("select[name='searchWord']").val('');
