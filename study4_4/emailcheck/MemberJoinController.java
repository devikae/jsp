...
//		return "cow"; 
		...
		public String mailAuth(String mail) throws MessagingException {
			...
			memberService.registMailAuth(mail, authKey);
		...
		public String authKeyCompare(MailAuthVO mailAuth) {
			...
			boolean authKeyCompare = memberService.authKeyCompare(mailAuth);
			
			if(authKeyCompare) {
				return "인증";
			}else {
				return "authKey가 다릅니다";
			}
		...
		public String isMailAuthed(String mail) {
			boolean isMailAuthed = memberService.isMailAuthed(mail);
			if(isMailAuthed) {
				return "인증";
			}else {
				return "인증X";
